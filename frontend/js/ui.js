...
// ... (Các hằng số element cũ giữ nguyên)
...
// (MỚI) Các element cho sidebar và nút lưu
...
function updateUI(data, lang) {
    const { location } = data;
    // (MỚI) Cập nhật trạng thái nút lưu sau khi có dữ liệu
...
// (MỚI) Các hàm cho sidebar và nút lưu
...
function renderFavoritesSidebar() {
    ...
    if (favorites.length === 0) {
        ...
        updateStaticText(localStorage.getItem('weatherAppLang') || 'en'); // Dịch ngay
        return;
    }
    favorites.forEach(city => {
        ...
        item.innerHTML = `<span>${city}</span> <button class="delete-favorite-btn" data-city="${city}">🗑️</button>`;
...
function updateSaveButtonState(city) {
    if (isFavorite(city)) {
        ...
        saveLocationBtn.innerHTML = '★'; // Sao vàng
    } else {
        saveLocationBtn.classList.remove('saved');
        saveLocationBtn.innerHTML = '☆'; // Sao rỗng
    }
}
