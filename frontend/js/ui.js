...
function displayTemperatures() {
    ...
    temperatureValueEl.innerHTML = `${Math.round(current['temp_' + currentUnit])}<sup>${unitSymbol}</sup>`;
    document.getElementById("realFeelAdditionalValue").innerHTML = `${Math.round(current['feelslike_' + currentUnit])}<sup>${unitSymbol}</sup>`;
...
function getUVDescription(uv, lang) {
    const translations = {
        'en': ['Low', 'Moderate', 'High', 'Very High', 'Extreme'],
        'vi': ['Thấp', 'Trung bình', 'Cao', 'Rất cao', 'Cực đoan'],
    };
    const levels = translations[lang] || translations['en'];
    if (uv <= 2) return levels[0];
    if (uv <= 5) return levels[1];
    if (uv <= 7) return levels[2];
    if (uv <= 10) return levels[3];
    return levels[4];
}

function getAQIDescription(aqi, lang) {
    const translations = {
        'en': ['', 'Good', 'Moderate', 'Unhealthy for SG', 'Unhealthy', 'Very Unhealthy', 'Hazardous'],
        'vi': ['', 'Tốt', 'Trung bình', 'Không tốt cho nhóm nhạy cảm', 'Không tốt', 'Rất không tốt', 'Nguy hiểm'],
    };
    const levels = translations[lang] || translations['en'];
